...
#### 一次查詢打幾次 TDX？免費方案能撐多少人？

一次位置查詢打 2 次 TDX 進階服務（路邊 NearBy、停車場 NearBy）。路段表是基礎服務，每縣市快取 6 小時；token 每把金鑰快取 6 小時；新北、基隆的市府資料不算 TDX。

基礎會員每月 3 點虛擬點數，點數算帳號不算金鑰，多申請金鑰只加每分鐘並發（每把 5 次／分），月總量不變。月上限 = 3 × 每點次數 ÷ 2，每點次數看 TDX「資料服務 > 進階服務」頁 NearBy 那列的綠色標籤：

| NearBy 計價 | 每月可查 | 每人每月 5 次全用滿 |
|------|------|------|
| 200 次／1 點 | 300 次 | 60 人 |
| 1,500 次／1 點 | 2,250 次 | 450 人 |

實際人數看平均用量。要分享出去就設 `MONTHLY_QUOTA` 和 `MONTHLY_BUDGET`，見 [3-C](#3-c-填金鑰兩條路都要做)。

#### 可以開放讓人拉進群組嗎？

可以，但額度是公用的，先設好上面兩個屬性。LINE Official Account Manager 的「允許加入群組／多人聊天」決定能不能拉；群組裡只回位置訊息，「id」指令不回。沒同意隱私政策的群組成員沒有 userId，只受 `MONTHLY_BUDGET` 限制。

#### 額度用完使用者會看到什麼？

個人額度用完回「本月 N 次額度已用完，下個月 1 號重置」，公共額度用完回「本月公共額度已用完」，兩種都附這份教學的連結。TDX 這一分鐘 5 次用完是另一句「查詢人數太多，請一分鐘後再試」，等一分鐘就好。

#### 使用者傳的位置會被存下來嗎？

不會。座標只用來查詢，不寫執行記錄，不存任何地方。額度計數只存 userId 加鹽雜湊後的前 16 位和次數，換月自動清掉。

#### 指令碼屬性裡多了一堆 `geo_…`、`quota_…`，那是什麼？會塞爆嗎？

`geo_地址` 是基隆 22 座公有停車場的地址反查結果（市府 CSV 沒座標），存 30 天後重查，這是 Google Maps Platform 對 geocode 結果的快取上限。`quota_年月_雜湊` 是當月每人的查詢次數，`quota_年月_total` 是合計，換月清掉。

指令碼屬性的上限是整庫 500KB、單一值 9KB、一天讀寫 50,000 次。基隆那 22 筆約 1.3KB，每個使用者的計數約 35 bytes，塞滿要一萬多個當月使用者，真的滿了新使用者會收到「本月的使用者人數統計已滿」，舊使用者不受影響；一次查詢動屬性 4 次左右，一天要一萬多次查詢才碰到讀寫上限。TDX 點數和 GAS 每天 20,000 次 `UrlFetchApp` 都會先用完。
