...
指令碼屬性的上限是整庫 500KB、單一值 9KB、一天讀寫 50,000 次。基隆那 22 筆約 1.3KB，每個使用者的計數約 35 bytes，塞滿要一萬多個當月使用者；一次查詢動屬性 4 次左右，一天要一萬多次查詢才碰到讀寫上限。TDX 點數和 GAS 每天 20,000 次 `UrlFetchApp` 都會先用完。
