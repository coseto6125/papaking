...
// 把板橋車站的實際回覆送到 LINE 的驗證端點（只檢查格式、不會發訊息），LINE 會指出 Flex 哪個欄位不合規
function testFlexValidate() {
  var messages = buildReply(25.014, 121.464).map(function (item) {
...
function testFull() {
  ...
  Logger.log('\n板橋車站:\n' + describeReply(buildReply(25.014, 121.464)));
